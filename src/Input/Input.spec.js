...
describe('enzyme testkit', () => {
  it('should exist', () => {
    ...
    const onChange = () => {};
    const dataHook = 'myDataHook';
    const wrapper = mount(<div><Input value={value} onChange={onChange} dataHook={dataHook}/></div>);
    const enzymeInputTestkit = enzymeInputTestkitFactory({wrapper, dataHook});
    expect(enzymeInputTestkit.exists()).toEqual(true);
  });
});
